...
(* Transposing operations
   ...
   by allowing as "proper" transpositions three cases:

     1. Transposing a push followed by a pop into a
        push, swap, pop sequence.
     2. Transposing a pop, pop, swap sequence into a pop and a pop.
     3. Transposing a swap, push, push sequence into a push and a push.
   ...
   transformation semigroup of permutations generated by the proper
   transpositions.
  *)
...
Definition irreducible_trm_pop_push_ops {T M} (op1 : pop_op T M) op2 :=
  match op1, op2 with
...
Arguments irreducible_trm_pop_push_ops {T M} !op1 !op2.

Definition transpose_trm_pop_push_left {T M} (op1 : pop_op T M) op2 :=
  match op1, op2 with
...
Arguments transpose_trm_pop_push_left {T M} !op1 !op2.

Definition transpose_trm_pop_push_right {T M} op2 (op1 : pop_op T M) :=
  match op1, op2 with
...
Arguments transpose_trm_pop_push_right {T M} !op2 !op1.

Lemma transpose_trm_pop_push {N T M}
      (op1 : pop_op T M) op2 (f : ivar (S N) T M) :
  irreducible_trm_pop_push_ops op1 op2 ->
  apply_pop_op op1
    (apply_push_op op2 f)
  =m=
    apply_push_op (transpose_trm_pop_push_left op1 op2)
      (swap_ivar
        (apply_pop_op (transpose_trm_pop_push_right op2 op1) f)).
Proof.
...
Ltac transpose_trm_pop_push op1 op2 :=
  let Hrw := fresh "Hrw" in
  epose (transpose_trm_pop_push op1 op2) as Hrw;
  cbn in Hrw; rewrite Hrw; clear Hrw.

Lemma transpose_trm_pop_push_squared_left {T M}
      (op1 : pop_op T M) op2 :
  transpose_trm_pop_push_left
    (transpose_trm_pop_push_left op1 op2)
    (transpose_trm_pop_push_right op2 op1)
  = op1.
Proof.
  intros.
...
Lemma transpose_trm_pop_push_squared_right op1 op2 :
  transpose_trm_pop_push_right
    (transpose_trm_pop_push_right op2 op1)
    (transpose_trm_pop_push_left op1 op2)
  = op2.
Proof.
...
Definition transpose_trm_pop_pop_left {T M} (op1 op2 : pop_op T M) :=
  match op1, op2 with
...
Arguments transpose_trm_pop_pop_left {T M} !op1 !op2.

Definition transpose_trm_pop_pop_right {T M} (op2 op1 : pop_op T M) :=
  match op1, op2 with
...
Arguments transpose_trm_pop_pop_right {T M} !op2 !op1.

Lemma transpose_trm_pop_pop {N T M} op1 op2 (f : ivar N T M) :
  apply_pop_op op1 (apply_pop_op op2 f)
  =m=
  swap_ivar
    (apply_pop_op (transpose_trm_pop_pop_left op1 op2)
      (apply_pop_op (transpose_trm_pop_pop_right op2 op1) f)).
Proof.
...
Lemma transpose_trm_pop_pop_squared_left {T M}
      (op1 op2 : pop_op T M) :
...
Proof.
  intros.
  destruct op1, op2; cbn; try easy.
  - transpose_iname_squared_left Del _ Del _.
    easy.
Qed.

Lemma transpose_trm_pop_pop_squared_right {T M}
      (op1 op2 : pop_op T M) :
...
Lemma transpose_trm_push_push {N T M} op1 op2 (f : ivar (S (S N)) T M) :
  apply_push_op op1
    (apply_push_op op2 f)
  =m=
...
Lemma transpose_trm_push_push_squared_left op1 op2 :
  transpose_trm_push_push_left
    (transpose_trm_push_push_left op1 op2)
    (transpose_trm_push_push_right op2 op1)
  = op1.
Proof.
  intros.
  destruct op1, op2; cbn; try easy.
...
Lemma transpose_trm_push_push_squared_right op1 op2 :
  transpose_trm_push_push_right
    (transpose_trm_push_push_right op2 op1)
    (transpose_trm_push_push_left op1 op2)
  = op2.
Proof.
